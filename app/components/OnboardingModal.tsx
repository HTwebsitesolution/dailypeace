...
async function maybeScheduleLocal(reminder: string) {
  try {
    if (!reminder) return;
    // Use the notifications service which handles permissions properly
    const { notifications } = await import("../../lib/notifications");
    const [h, m] = reminder.split(":").map(Number);
    
    // Request permissions first (user-initiated via onboarding)
    const granted = await notifications.requestPermissions();
    if (!granted) {
      console.warn("Notification permissions not granted, skipping schedule");
      return;
    }
    
    // Schedule using the proper service method
    await notifications.scheduleDailyVerse(h, m);
  } catch (error) {
    console.warn("Failed to schedule notification in onboarding:", error);
